...
function transitionToLinePlots() {
    scaleY = d3.scaleLinear()
        ...
        .range([(ctx.totalLinePlotHeight - ctx.timeAxisHeight)/CITIES.length, ctx.vmargin])
    dark_grey = "#333333"
    // Fade out the legend
    d3.select("#tempLegend")
        ...
        .end().then(function(){ 
            d3.selectAll(".plot")
                .transition()
                .duration(ctx.STAGE_DURATION)
                .attr("transform", (d, i) => `translate(${ctx.hmargin},${i * (ctx.h-ctx.timeAxisHeight)/CITIES.length})`)
            d3.select("#timeAxis")
            .transition()
            .duration(ctx.STAGE_DURATION)
            .attr("transform", `translate(${ctx.hmargin},${ctx.totalLinePlotHeight-ctx.timeAxisHeight})`)
            .end().then(() => (
                // Tarnslate all color strips, labels, as well as the time axis to fill the entire height of the SVG canvas
...
function transitionToColorStrips() {
    // Translate all color strips, labels, as well as the time axis to their original positions
    plots = d3.selectAll(".plot")
    plots.each(function(plot) {
        d3.select(this).selectAll("line")
            .transition()
            ...
            .transition()
            .delay((d, i) => (i * 4 + ctx.STAGE_DURATION))
            .duration(ctx.STAGE_DURATION)
            .attr("stroke", (d) => (ctx.color(d)))
            .attr("y1", ctx.vmargin)
            .attr("y2", ctx.BAND_H - ctx.vmargin)
            .end().then(function(){
                ...
                d3.select("#timeAxis")
                    .transition()
                    .duration(ctx.STAGE_DURATION)
                    .attr("transform", `translate(${ctx.hmargin},${ctx.totalStripHeight-ctx.timeAxisHeight})`)
                    .end().then(function(){
                        d3.select("#tempLegend")
                            .transition()
                            .duration(ctx.STAGE_DURATION)
